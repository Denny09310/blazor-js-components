...
export function afterStarted(blazor) {
    ...
    if (!globalThis.__blazorJSReviverAttached) {
        globalThis.DotNet.attachReviver(reviveJSComponentArgs);
        globalThis.__blazorJSReviverAttached = true;
    }

    let isNavigating = false;

    if (!globalThis.__blazorJSNavListenersAttached) {
        ...
        globalThis.__blazorJSNavListenersAttached = true;
    }

    if (!customElements.get("bl-script")) {
...
function reviveJSComponentArgs(_key, value) {
    if (
        ...
        for (const element of elements) {
            const attr = element.dataset.ref;
...
function invokeJSComponentMethod(instanceId, identifier, args) {
    ...
    if (typeof method !== "function") {
        throw new Error(`The JS component does not define method '${identifier}'`);
    }
